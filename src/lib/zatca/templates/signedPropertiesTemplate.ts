...
// Keep the emitted whitespace identical to the hashing template. Only namespace
// declarations and the empty DigestMethod representation differ.
const templateFinal = templateForSigning
  .replace(/ xmlns:ds="http:\/\/www\.w3\.org\/2000\/09\/xmldsig#"/g, "")
  .replace(/(<ds:DigestMethod[^>]+)\/>/, "$1></ds:DigestMethod>");
