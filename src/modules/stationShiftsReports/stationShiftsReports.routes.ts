...
import { salesVolumeHandler, cashSummaryHandler, expensesHandler, netCashHandler } from "./stationShiftsReports.controller";
 ...
 *
 * صافي النقدية استثناء وحيد متعمَّد: تُضاف صلاحية "review" نفسها المستخدَمة في stationShifts.routes.ts
 * لشاشة المحاسب — نفس مبدأ ذلك الملف الذي يخصّص مستوى صلاحية مختلفاً لكل مسار على حدة (reviewAccess
 * مقابل postAccess) بدل صلاحية واحدة موحَّدة لكل الراوتر. لا مسار من بوابة الموظف (authenticateEmployeePortal)
 * يصل لهذا الملف إطلاقاً — هذا الراوتر لا يُركَّب إلا تحت authenticate (User/Position) في app.ts.
 */
