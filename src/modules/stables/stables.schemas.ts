...
export const createHorseSchema = z.object({
  ...
  ownerName: optionalText, ownerPhone: optionalText, ownerEmail: z.string().trim().email().optional().nullable(), ownerNationalId: optionalText,
  monthlyServices: z.array(z.object({ serviceId: z.string(), trainerId: optionalText, quantity: z.coerce.number().positive().default(1), unitPrice: optionalMoney })).optional(),
...
export const trainerSchema = z.object({ companyId:z.string(), name:z.string().trim().min(2), phone:optionalText, email:z.string().email().optional().nullable(), specialties:optionalText, bio:optionalText, commissionPct:z.coerce.number().min(0).max(100).default(5), isActive:z.boolean().optional() });
...
export const lessonSchema = z.object({ companyId:z.string(), lessonTypeId:z.string(), trainerId:z.string(), horseId:optionalText, studentName:z.string().trim().min(2), studentPhone:optionalText, studentEmail:z.string().trim().email().optional().nullable(), scheduledAt:z.coerce.date(), participants:z.coerce.number().int().positive(), price:z.coerce.number().nonnegative(), status:z.enum(["scheduled","completed","cancelled","no_show"]).optional(), notes:optionalText });
...
export const createBoardingInvoiceSchema = z.object({ companyId:z.string(), customerId:z.string(), billingMonth:z.string().regex(/^\d{4}-\d{2}$/), trainerSelections:z.record(z.string()).optional() });
